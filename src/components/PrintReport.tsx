  ...
  return (
    <article id="report-print-area" className="print-report report-print-document">
      ...
      {reportType === "sales-history" && <>
        ...
        <Section title="Sales History"><table><thead><tr><th>Date</th><th>Sale #</th><th>Customer</th><th className="num">Total</th><th className="num">Paid</th><th className="num">Due</th><th>Payment</th><th>Return</th></tr></thead><tbody>{sales.length === 0 ? <EmptyRow columns={8}>No sales in this period.</EmptyRow> : sales.map((item) => <tr key={item.id}><td>{formatDate(item.created_at)}</td><td>{item.receipt_no}</td><td>{item.member_name ?? "Walk-in"}</td><td className="num">{formatMoney(item.total_amount)}</td><td className="num">{formatMoney(item.paid_amount)}</td><td className="num">{formatMoney(Math.max(0, item.total_amount - item.paid_amount - (item.returned_amount ?? 0)))}</td><td>{methodLabels[item.payment_method] ?? item.payment_method}</td><td>{item.return_status ?? "none"}</td></tr>)}</tbody></table></Section>
