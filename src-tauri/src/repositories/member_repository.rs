...
pub fn insert(conn: &Connection, input: &CreateMemberInput) -> Result<i64, AppError> {
    conn.execute(
        "INSERT INTO members (name, phone, email, address, notes)
         VALUES (?1, ?2, ?3, ?4, ?5)",
        params![
            input.name,
            input.phone,
...
pub fn get_by_id(conn: &Connection, id: i64) -> Result<Option<Member>, AppError> {
    let row = conn
        .query_row(
            "SELECT id, name, phone, email, address, image_path, status, notes, is_deleted, created_at, updated_at
...
pub fn list(conn: &Connection, search: Option<&str>, limit: i64) -> Result<Vec<Member>, AppError> {
    ...
    if has_search {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, name, phone, email, address, image_path, status, notes, is_deleted, created_at, updated_at
             FROM members
             WHERE is_deleted = 0 AND (name LIKE ?1 OR phone LIKE ?1 OR email LIKE ?1)
...
pub fn update(
    ...
    let affected = conn.execute(
        "UPDATE members SET name = ?1, phone = ?2, email = ?3, address = ?4, notes = ?5, updated_at = CURRENT_TIMESTAMP
