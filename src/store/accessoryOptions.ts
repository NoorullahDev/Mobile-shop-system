...
import * as accessoryOptionService from "../services/accessoryOptionService";

interface AccessoryOptionsState {
  options: PhoneOption[];
  loading: boolean;
  error: string | null;
  load: () => Promise<void>;
...
export const useAccessoryOptionStore = create<AccessoryOptionsState>((set, get) => ({
  options: [],
  loading: false,
  error: null,
  ...
  load: async () => {
    ...
    try {
      const options = await accessoryOptionService.listAccessoryOptions();
  ...
  add: async (input, actor) => {
    set({ error: null });
    try {
      const created = await accessoryOptionService.createAccessoryOption(input, actor ?? null);
  ...
  update: async (id, input, actor) => {
    set({ error: null });
    try {
      const updated = await accessoryOptionService.updateAccessoryOption(id, input, actor ?? null);
      set((s) => ({
  ...
  remove: async (id, actor) => {
    set({ error: null });
    try {
      await accessoryOptionService.deleteAccessoryOption(id, actor ?? null);
